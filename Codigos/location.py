...
def calle2ubicacion():
    ...
    while True:
        ...
        if len(data)==0:
            ...
            while True:
                ciudad = input("Introdue la ciudad en la que estás por favor: ")
                params['city']=ciudad
                response_calle = requests.get(BASE_URL, params=params)
                data = response_calle.json()
                if len(data)==0:
    ...
    if aeropuerto['place_id'] != barajas_place_id:
        ...
        while True:
            ...
            if llevar_a_barajas.lower() == "si":
                ...
                print(f"El aeropuerto {direcion_final[0]} se encuentra a una distancia de {distancia}.")
                print(f"Actualmente tardaría {duracion} en llegar.")
                mapa = folium_map(ruta)
                return mapa

            elif llevar_a_barajas.lower()=="no":
                print("Sentimos nos tener su aeropuerto. Lo añadiremos en el futuro.\n Gracias por utilizar nustros servicios.")
                return
...
def dibujar_mapa(lista_sitios, ciudad):
    ...
    mapa = folium.Map(location=punto_medio, zoom_start=11, width=800, height=500)
